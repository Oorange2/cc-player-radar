...
local function itemListUI(cfg)
    ...
    local function draw()
        ...
        if fetchErr and #filtered == 0 then
            ...
            local s = fetchErr
            while #s > 0 do
                table.insert(lines, s:sub(1, W)) s = s:sub(W+1)
            end
            for i, line in ipairs(lines) do
                if i+2 <= H-1 then
                    term.setCursorPos(1, i+2) term.write(line)
                end
            end
        else
            for row = 1, H-2 do
    ...
    while true do
        ...
        if searchMode then
            ...
            elseif event=="timer" and p1==postTimer then
                postTimer=nil doFetch() applyFilter()
            end
        end
    end
end
...
local function logScreen()
    ...
    while true do
        ...
        elseif ev=="mouse_scroll" then scroll=math.max(0,math.min(scroll+p1,math.max(0,#log-(H-2)))) end
    end
end
        ...
        elseif p1==keys.enter then
            ...
            elseif sel==4 then
                ...
                while true do
                    ...
                    for row=1,H-2 do
                        local n=names[row+ds] term.setCursorPos(1,row+1) term.setBackgroundColor(colors.black)
            ...
            elseif sel==5 then token=nil username=nil isAdmin=false return end
        end
    end
end

...
local function itemListUI(cfg)
    ...
    local function applyFilter()
        ...
        if searchQuery == "" then filtered = items
        else
    ...
    while true do
        ...
        if searchMode then
            if event == "char" then searchQuery = searchQuery .. p1 applyFilter()
            ...
            if event == "key" then
                if p1 == keys.leftShift or p1 == keys.rightShift then shiftHeld = true
                elseif p1 == keys.up then
                    if selected > 1 then selected = selected-1
                        if selected <= scroll then scroll = scroll-1 end end
                elseif p1 == keys.down then
                    if selected < #filtered then selected = selected+1
                        if selected > scroll+(H-2) then scroll = scroll+1 end end
                elseif p1 == keys.enter then
                    local stack = shiftHeld shiftHeld = false
                    ...
                    if item and not cfg.readOnly and cfg.actionFn then
                        ...
                        else message = err or "Failed" msgTimer = os.clock() + 3 end
                    end
                elseif p1 == keys.r then items = cfg.fetchFn() applyFilter() message="Refreshed" msgTimer=os.clock()+1
            ...
            elseif event == "mouse_scroll" then
                if shiftHeld then
                    ...
                    if p1 == -1 and selected > 1 then selected = selected-1
                        if selected <= scroll then scroll = scroll-1 end
                    elseif p1 == 1 and selected < #filtered then selected = selected+1
                        if selected > scroll+(H-2) then scroll = scroll+1 end
                    end
                end
            elseif event == "timer" and p1 == postTimer then
                postTimer = nil items = cfg.fetchFn() applyFilter()
            end
        end
    end
end
...
local function logScreen()
    ...
    while true do
        term.setBackgroundColor(colors.black) term.clear()
        term.setBackgroundColor(colors.blue) term.setTextColor(colors.white)
        term.setCursorPos(1,1) term.clearLine() term.write(" Activity Log [" .. #log .. " entries]")
        for row = 1, H-2 do
        ...
        elseif p1 == keys.enter then
            if sel == 1 then
                itemListUI({ title="Withdraw", actionLabel="Withdrew",
                    fetchFn=function()
                        local r = rpc({ type="list_vault", token=token }) return (r and r.items) or {} end,
            ...
            elseif sel == 2 then
                itemListUI({ title="Deposit", actionLabel="Deposited",
                    fetchFn=function()
                        local r = rpc({ type="list_inventory", token=token }) return (r and r.items) or {} end,
                    actionFn=function(item, amt)
...
local function adminMenu()
    local opts = { "List Users", "Create User", "Manage User", "Logout" }
    local sel  = 1
    ...
    while true do
        ...
        term.setCursorPos(1,H) term.setBackgroundColor(colors.black)
        if msg2 ~= "" and os.clock() < mt2 then term.setTextColor(colors.lime) term.write(msg2:sub(1,W))
        else msg2="" end
        local ev, p1 = os.pullEvent("key")
        if p1 == keys.up and sel > 1 then sel = sel-1
        elseif p1 == keys.down and sel < #opts then sel = sel+1
        elseif p1 == keys.enter then
            if sel == 1 then
                ...
                while true do
                    ...
                    for row = 1, H-2 do
                        local u = users[row+us] term.setCursorPos(1, row+1) term.setBackgroundColor(colors.black)
                        if u then
                ...
                if target ~= "" then
                    ...
                    while true do
                        ...
                        elseif p3 == keys.enter then
                            ...
                            elseif subSel == 6 then break end
                        end
                    end
                end
            elseif sel == 4 then token=nil username=nil isAdmin=false return end

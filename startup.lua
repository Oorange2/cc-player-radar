local URL  = "https://raw.githubusercontent.com/Oorange2/cc-player-radar/main/cloud_user.lua"
local FILE = "cloud_user.lua"

local function readLocal()
    if not fs.exists(FILE) then return nil end
    local f = fs.open(FILE,"r"); local c = f.readAll(); f.close(); return c
end
